...
class RouterSet[+FoundRoute](routers: Seq[Router[FoundRoute]]) extends Router[FoundRoute] {
  def apply(requestParts: Seq[String]): Option[FoundRoute] = {
    routers.iterator.map(_(requestParts)).find(_.isDefined).map(_.get)
  }
}
...
class SubRouter[+FoundRoute](prefix: Seq[String], subRouter: Router[FoundRoute]) extends Router[FoundRoute] {
  ...
  def apply(requestParts: Seq[String]): Option[FoundRoute] = {
    if(requestParts.startsWith(prefix)) subRouter(requestParts.drop(pfxLen))
    else None
  }
}
